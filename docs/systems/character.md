...
- **Resource**: zjednodušený `ResourceType` (mana/energy/rage) **scrapnut** (ADR 0034) — D&D resources: spell sloty (caster, `spell-slots.ts`), **Ki** (Monk) / **Rage** (Barbarian) / **Pact** (Warlock) v `class-resources.ts`. `DerivedStats` nese `kiPoints`/`rageCharges`.
