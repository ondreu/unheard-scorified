...
- [ ] **Akční ekonomika — action surge, bonus action, „once per combat"** 🚧 _(D&D action economy, ADR 0042; krájeno na slice, sjednoceno napříč všemi 6 simulátory)_:
  ...
  - [ ] **Slice 3 — bonus action jako samostatný akční slot** (`actionCost: 'action' | 'bonus'`) — 1 akce + 1 bonus action / kolo; napojení do rotace, tahových turn-loopů a interaktivního UI (dungeon/gauntlet). Sem spadne i **`oncePerTurn`** (Sneak Attack se přičte jen k jednomu útoku v kole, jakmile útoků v kole může být víc).
