...
- [ ] **Akční ekonomika — action surge, bonus action, „once per combat"** 🚧 _(D&D action economy, ADR 0042; krájeno na slice, sjednoceno napříč všemi 6 simulátory)_:
  ...
  - [ ] **Slice 2 — Action Surge / Onslaught jako reálná akce navíc** (`grantsExtraAction`) — extra útok v tomtéž kole; tady začne platit **`oncePerTurn`** (Sneak Attack se přičte jen k jednomu útoku v kole).
  - [ ] **Slice 3 — bonus action jako samostatný akční slot** (`actionCost: 'action' | 'bonus'`) — 1 akce + 1 bonus action / kolo; napojení do rotace, tahových turn-loopů a interaktivního UI (dungeon/gauntlet).
