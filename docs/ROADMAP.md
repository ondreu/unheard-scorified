...
4. **Server tick loop** jen pro *živé* události vyžadující průběžný update (probíhající dungeon/raid/arena boj, regenerace, world eventy) — tick každých N s, výsledky přes WebSocket pub/sub. Boj je idle/auto-resolve (auto-attack + cooldowny dle talentů/gearu).
...
### M0 — Skeleton & infra (základ) — 🚧 probíhá
- [ ] Monorepo (pnpm + turbo), TypeScript config, lint/format.
- [ ] docker-compose: api + web + postgres + redis + Caddy (HTTPS).
- [ ] NestJS skeleton + healthcheck; SvelteKit skeleton + PWA manifest + service worker.
- [ ] Drizzle setup + první migrace; připojení k Redisu.
- [ ] CI základ (build/test/lint, brání mergi při červené), SessionStart hook pro web sezení.
- [ ] Agent-first základ: kořenový `CLAUDE.md` + šablona modulu, `docs/adr/` s prvními ADR, striktní TS/ESLint/Prettier, `pnpm dev` + seed skript.
- [ ] Stateless API od začátku (stav v Postgres/Redis), připravené na horizontální škálování.
- **Výstup:** prázdná appka běží v Dockeru, instalovatelná jako PWA; nový agent se zorientuje z `CLAUDE.md`.
