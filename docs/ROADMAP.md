...
> 🔜 **Handoff:** hotovo **MR-1 → MR-7**, **celý cluster deWoWčení** (frakce ✅, PHB rasy ✅, lore „The Caldmoor Reaches" ✅) a **MR-11 — Level cap 20 + XP křivka** ✅ (cap 20, překalibrovaná křivka, obsah přeškálován do 1–20, hra plně hratelná). Combat má D&D dice-roll (MR-5) + bestiář/CR/typové poškození (MR-7). **MR-10 rozjeté po slice:** ✅ **MR-10a** (CR-based enemy AC/attackBonus/save DC) + ✅ **MR-10b** (per-class weapon dice + typové útoky) + ✅ **MR-10c** (loot rarity-driven + arena rescale) + ✅ **MR-10d** (per-ability typy + typovaný late-game obsah). Zbývá: **Onboarding/tutoriál** (redesign po D&D char creation) a poslední kus **MR-10 — Balance pass** (literal per-spell dice 8d6 Fireball nezávisle na attackPower + CR-based HP/damage magnitudy) — záměrně až nakonec.
