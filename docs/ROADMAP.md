...
- [ ] **Fix kouzla nesedící na D&D** — narovnat ability, které neodpovídají D&D mechanice (např. **Hunter's Mark** = momentálně jen weapon strike; v D&D je to concentration buff +1d6 na zásah). Audit celého katalogu `abilities.ts` proti D&D 5e.
  - 🔥 **Priorita / blocker z balance passu (ADR 0035):** caster **magnitudy**. Leveled kouzla i cantripy běží na literal D&D kostkách (Fire Bolt 1d10, Fireball 8d6) hluboko pod martial `attackPower` (~40–70/úder) a cantripy neškálují s levelem → po D&D-ifikaci base (zrušen per-level růst skóre) **wizard/cleric nejsou viable** na high-level bossech. Náprava: cantrip scaling (D&D 1→2→3→4 kostek na 5/11/17) + zvednout sustained leveled kouzel tak, aby všechny classy spadly do cíle ~6–12 úderů / win s HP ztrátou (ověřit harnessem `gear-balance.test.ts`).
