...
- [ ] **Enemy schopnosti** 🚧 — aktivní abilities nepřátel (ne jen boss `damageMult`/special): typované útoky, conditiony, saving-throw efekty proti hráči.
  ...
  - [ ] **Slice 2 — content + conditiony** — přiřadit abilities dungeon bossům + bestiáři (rebalance proti `gear-balance`), implementovat conditiony (stun = vynech tah, frightened/slow = disadvantage/zpomalení, prone…) + Gauntlet draw abilit z katalogu + drain/dot enemy ability.
