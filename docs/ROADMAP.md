...
- [x] **Akční ekonomika — action surge, bonus action, „once per combat"** ✅ _(D&D action economy, ADR 0042; krájeno na 3 slice)_:
  ...
  - [x] **Slice 3 — bonus action jako samostatný akční slot** ✅ — `SignatureAbility.actionCost` (`'action'` default / `'bonus'`) + sdílený `isBonusAction`; **Healing Word / Mass Healing Word** otagovány jako bonus action. V **tahových group PvE simech** (dungeon-run solo+AI parťáci, dungeon-party MP) aktér po hlavní akci automaticky provede **jednu ready bonus-action ability** v tomtéž kole (D&D „1 akce + 1 bonus / kolo"); rotation/cooldown-gated, jen 1 bonus/kolo (Healing Word použitý jako hlavní akce se neduplikuje). Bonus je auto-resolved (jako rage), interaktivní výběr bonus akce v UI = follow-up. **Gauntlet vynechán schválně** (roguelite heal-scarcity design by free bonus heal rozbil). Spojité (timeline) simy: `actionCost` kosmetický (žádné kolo). **`oncePerTurn` záměrně nezaváděno** — Sneak Attack je v enginu diskrétní ability (1× za aktivaci) a bonus akce je cap 1/kolo, takže pravidlo „1× za kolo" je strukturálně splněné (viz ADR 0042). Testy `action-economy.test.ts` zelené.
  - _Follow-up (mimo tento ADR): interaktivní výběr bonus akce v UI (dungeon/gauntlet), bonus action v Gauntletu, další bonus-action ability (Misty Step, Cunning Action, two-weapon)._
