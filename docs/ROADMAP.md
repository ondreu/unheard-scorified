...
- [ ] **Výběr targetů v combatu** — rozšířit cílení napříč režimy (krájeno na slice):
  - [x] **Friendly targeting (tahové dungeony)** ✅ — hráč volí **cíl léčení** (heal ability) mezi členy party místo auto-`mostInjured`. Sjednoceno: solo+AI parťáci (`dungeon-run.ts` `resolveHealTarget`, `targetId` = index člena 0=self/1..N parťák) i MP (`dungeon-party.ts` `resolveHealTarget`, `targetId` = `slot` člena). Význam `targetId` jednoznačný dle `ability.kind` (heal = spojenec, jinak nepřítel) → **bez změny API tvaru / DB migrace**; neplatný/mrtvý cíl → fallback nejzraněnější (zpětně kompatibilní). AI healer cílí explicitně `mostInjured`. Web: klik na kartu hráče/parťáka (dungeon-turn + dungeon-party) volí heal cíl (ring + „💚 Heal target"). Kontraktní testy v `dungeon-run.test.ts`/`dungeon-party.test.ts`. _Shield/mitigation zatím self-buff (cílení spojenců = follow-up)._
