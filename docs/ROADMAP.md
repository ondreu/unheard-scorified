...
- [ ] **Level-up overhaul — víc D&D, přehlednější** 🔜 _(vyčleněno z Knihy kouzel — rozhodnutí PM; velké → ADR)_ — současný Level Up (`levelup.ts`, `data/feats.ts`, web `/levelup`) dává **volbu jen na 5 levelech** (subclass + ASI/Feat na 4/8/12/16/19); na zbylých ~14 levelech se na obrazovce neděje **nic** (žádný feedback, co level přinesl) a **featy jsou jeden plochý globální seznam** abstraktních combat-tagů dostupný všem třídám stejně (ne moc D&D). Cíl PM: **víc D&D-like a přehledné.** Krájeno na slice (pořadí s PM):
  - [ ] **Slice A — UI redesign (přehlednost).** Předělat `/levelup` na čitelný **level track 1–20**: u každého levelu ukázat, co přináší (HP z hit dice, proficiency bonus, nové spell sloty, nově dostupná kouzla do Knihy kouzel, class feature) — **každý level = událost s feedbackem**, ne prázdno. Milníkové volby (subclass / ASI / Feat) vizuálně zvýrazněné. Mobile-first (PWA). Čistá prezentace existujících dat — bez změny mechanik.
  - [ ] **Slice B — D&D-věrné featy (podle class).** Přepsat `data/feats.ts` z abstraktních combat-tagů na **D&D 5e feat roster**: half-featy (**+1 do atributu** navíc k efektu), prerekvizity (atribut/level/caster), a **filtrování/seskupení dle classy** (martialové vidí relevantní bojové featy, casteři caster featy…). Efekty navázat na reálné D&D mechaniky (engine z ADR 0036), ne jen `dmg_minor`. _❓ k rozhodnutí s PM: plný D&D feat list vs. kurátorský; zda i **class-feature volby** (Fighting Style pro Fighter/Paladin/Ranger, Metamagic pro Sorcerera, Eldritch Invocations pro Warlocka, Battle Master maneuvery) jako součást level-upu._
  - [ ] **Slice C — class features po levelech (D&D progrese).** Doplnit datově **class features** na jejich D&D levelech (Extra Attack, Rage uses, Ki, Sneak Attack scaling, Channel Divinity uses…) tak, aby se ukázaly v level tracku a kde dávají volbu, byly volitelné. Sjednotit s abilit kitem (`resolveAbilities`/Kniha kouzel) a `class-resources.ts`. _Pozn.: část už existuje implicitně (scaling v abilities) — sem patří zviditelnění + chybějící volby._
