...
- [x] **MR-5 — dice-roll combat** ✅ (d20 vs AC, damage dice, saving throws, initiative):
      - **Sdílená dice primitiva**: `dice.ts` (d20, NdX, attack roll s nat 20/nat 1,
        saving throws, notace) + `dnd-combat.ts` (`savingThrow`, `rollInitiative`,
        log-buildery + `missMessage`/`rollTag`). Jádro hodu (`rollHit`/`computeHit`/
        `resolveAttack`) + actor helpery + damage dice v `combat.ts`. Deterministické.
      - **`CombatActor` + D&D pole**: `armorClass`/`attackBonus`/`damageBonus`/
        `saveMods`/`spellSaveDc`/`spellSlots`, z `deriveCombatProfile` (hráč) resp.
        `buildEnemyActor` (nepřítel — AC/attackBonus z `EnemyStats.level` ~level/2:
        raid z requiredLevel, dungeon z requiredLevel, Gauntlet z level+wave).
      - **Increment 1 — quest combat**: initiative (d20+DEX), hod na zásah vs AC
        (log `rolls 14 + 6 = 20 vs AC 13 → HIT for 28 damage`), saving throws (hráč
        spell → enemy CON save; boss special → hráč DEX save), **spotřeba spell slotů
        z MR-4** (rozpočet běhu, fallback na zbraň/cantrip → `casts Fireball
        (3rd-level slot)`). Combat-objective gated buildem zachován.
      - **Increment 2 — sjednocení všech simulátorů**: `computeHit` přepsán na dice →
        **dungeon/raid/PVP/aréna/Gauntlet** běží na jednom D&D modelu (žádné dva
        modely). Miss = `amount: 0` s miss-aware logy (`[d20: … vs AC …]`).
      - Testy: shared `dice` (+11), `dnd-combat` (+11), quest-run (+3), raid/PVP
        dice-log asserty (+2). Build/test/lint/typecheck zelené (408 shared + 192 API).
        **ADR 0030**, `docs/systems/dnd-combat.md`.
      - _Balanc (rozhodnutí PM: až nakonec převzetím D&D čísel) = MR-10: plný
        damage-dice redesign per zbraň/kouzlo (1d8+STR, 8d6 Fireball), CR-based
        AC/HP/damage, save-DC kouzla, recalibrace XP/Elo/wipe._

> 🔜 **Handoff pro další session:** hotovo **MR-1, MR-2 (+MR-6), MR-3, MR-4, MR-5**.
> Pokračovat **MR-7** (D&D bestiář + CR: enemy katalog `data/enemies.ts` o víc typů
> s resistancemi/zranitelnostmi, Challenge Rating), pak MR-8 (lore přejmenování
> zón/dungeonů/raidů — WoW → homebrew D&D), MR-9 (odstranění frakcí), MR-10 (balance
> pass — **převzetí D&D čísel**: damage dice 1d8+STR/8d6, CR, XP tabulka, spell
> economy, Elo/wipe recalibrace), MR-11 (level cap 20 + XP křivka). Pozn.: rasy jsou
> stále WoW sada namapovaná na D&D atributy — PHB rasy přijdou s lore přejmenováním
> (MR-8/MR-9).
