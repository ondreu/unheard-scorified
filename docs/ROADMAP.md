...
> 🔜 **Handoff:** hotovo **MR-1 → MR-7** + **celý cluster deWoWčení** (odstranění frakcí ✅, PHB rasy ✅, lore přejmenování na „The Caldmoor Reaches" ✅). Combat má D&D dice-roll (MR-5) + bestiář/CR/typové poškození (MR-7). Přirozený další krok = **MR-11 — Level cap 20 + XP křivka** (případně Onboarding). Balanc (MR-10) — převzetí D&D čísel + přemapování obsahu na bestiář/CR — záměrně až nakonec.
