...
describe('isValidRaceClass', () => {
  it('povolí vanilla kombinaci (Night Elf Druid)', () => {
    expect(isValidRaceClass('nightelf', 'druid')).toBe(true);
  });

  it('zakáže nevalidní kombinaci (Human Druid)', () => {
    expect(isValidRaceClass('human', 'druid')).toBe(false);
  });
...
describe('baseStatsFor', () => {
  it('aplikuje rasové i class modifikátory', () => {
    const taurenWarrior = baseStatsFor('tauren', 'warrior', 1);
...
describe('deriveStats', () => {
  it('mana classa má manu odvozenou z intellectu', () => {
    const primary = baseStatsFor('gnome', 'mage', 1);
    const d = deriveStats(primary, 1, 'mage');
    expect(d.resource.type).toBe('mana');
    expect(d.resource.max).toBeGreaterThan(100);
  });

  it('warrior používá rage (fixní 100)', () => {
    const primary = baseStatsFor('orc', 'warrior', 1);
    expect(deriveStats(primary, 1, 'warrior').resource).toEqual({ type: 'rage', max: 100 });
  });
});

describe('buildCharacterSheet', () => {
  it('0 XP = level 1 a konzistentní sheet', () => {
    const sheet = buildCharacterSheet('orc', 'shaman', 0);
